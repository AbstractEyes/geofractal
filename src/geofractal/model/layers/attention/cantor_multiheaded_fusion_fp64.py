...
"""
CantorMultiheadFusion - FULLY OPTIMIZED WITH PROPER BUFFER MANAGEMENT
----------------------------------------------------------------------
Optimizations:
    1. All geometric structures as properly registered buffers
    2. FP64 Cantor staircase creation → FP32 runtime (precision + speed)
    3. Residual scale as buffer for optimization
    4. Proper dtype conversion throughout
    5. Routes computed in FP64, stored in model dtype
"""
...
class CantorFusionConfig:
    ...
    # Dtype management
    dtype: torch.dtype = torch.float32  # Runtime dtype
    geometric_dtype: torch.dtype = torch.float64  # High-precision geometric computation
    ...
    use_optimized_gather: bool = True
    ...
    def __post_init__(self):
        ...
        if self.simplex_config is None:
            ...
            self.simplex_config = SimplexConfig(
                k_simplex=k,
                use_barycentric=self.use_barycentric,
...
class CantorMultiheadFusion(nn.Module):
    """
    Cantor Multihead Sparse Fusion - FULLY OPTIMIZED WITH BUFFER MANAGEMENT

    Key optimizations:
        ✅ Geometric computations in FP64 for precision
        ✅ Runtime operations in FP32/model dtype for speed
        ✅ All structures as properly registered buffers
        ✅ Residual scale as buffer for torch optimization
    ...
    def __init__(self, config: CantorFusionConfig):
        ...
        self.validate_geometry = config.validate_geometry

        print(f"[CantorFusion] Initializing OPTIMIZED version...")
        print(f"  Runtime dtype: {config.dtype}")
        print(f"  Geometric dtype: {config.geometric_dtype} (high precision)")
        ...
        # Residual scale as buffer (not parameter - no gradients needed)
        self.register_buffer(
            'residual_scale_buffer',
            torch.tensor(config.residual_scale, dtype=config.dtype),
            persistent=False
        )

        # Epsilon as buffer for numerical stability
        self.register_buffer(
            'eps_buffer',
            torch.tensor(config.eps, dtype=config.dtype),
            persistent=False
        )
        ...
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        # PRE-COMPUTE GEOMETRIC STRUCTURES
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

        start_time = time.time()

        # Pre-compute Beatrix staircase (FP64 → FP32)
        ...
        # Pre-compute routing tables (FP64 → FP32)
        if config.precompute_routes:
            self._prebuild_routing_tables()

        # Pre-compute distance matrices (FP64 → FP32)
    ...
    def _prebuild_staircase_cache(self):
        ...
        Computed in FP64 for geometric precision, stored in runtime dtype for speed.
        """
        print(f"[CantorFusion] Pre-computing Beatrix staircase (FP64 → FP32)...")
        ...
        for seq_len in self.config.staircase_cache_sizes:
            ...
            # Compute in FP64 for precision
            cantor_measure_fp64, features_fp64 = factory.build(
                backend="torch",
                device="cpu",
                dtype=self.config.geometric_dtype,  # FP64
                validate=self.validate_geometry
            )

            # Convert to runtime dtype for speed
            cantor_measure = cantor_measure_fp64.to(dtype=self.config.dtype)
            features = features_fp64.to(dtype=self.config.dtype)

            # Register as buffers (persistent=False for no checkpoint bloat)
            self.register_buffer(
    ...
    def _prebuild_routing_tables(self):
        ...
        Routes computed in FP64 for distance precision, stored as int64.
        """
        print(f"[CantorFusion] Pre-computing routing tables (FP64 → int64)...")
        ...
        for seq_len, k in precompute_sizes:
            # Build routes (computed with FP64 distances)
            routes = self._build_fusion_routes(seq_len, k)

            # Routes are indices - store as int64
    ...
    def _prebuild_distance_matrices(self):
        ...
        Computed in FP64 for precision, stored in runtime dtype.
        """
        print(f"[CantorFusion] Pre-computing distance matrices (FP64 → FP32)...")

        for seq_len in self.config.staircase_cache_sizes:
            if seq_len > self.config.max_seq_len:
                continue

            # Build distance matrix in FP64
            factory = self._get_or_create_factory(seq_len, RouteMode.DISTANCE)
            distance_matrix_fp64 = factory.build(
                backend="torch",
                device="cpu",
                dtype=self.config.geometric_dtype,  # FP64
                validate=self.validate_geometry
            ...
            # Convert to runtime dtype
            distance_matrix = distance_matrix_fp64.to(dtype=self.config.dtype)

            self.register_buffer(
                f"_distances_{seq_len}",
                distance_matrix,
                persistent=False
            )
    ...
    def _get_cached_staircase(
        ...
        """
        Get Beatrix staircase from cache - instant buffer lookup.

        Returns tensors already in runtime dtype and on correct device.
        """
        ...
        if hasattr(self, buffer_name_cantor):
            # Cache hit! Buffers automatically on correct device
            cantor = getattr(self, buffer_name_cantor)
            features = getattr(self, buffer_name_features)
            return cantor, features

        # Cache miss - compute on demand with proper dtype handling
        factory = self._get_or_create_factory(seq_len, RouteMode.STAIRCASE_FEATURES)
        ...
        cantor_measure_fp64, features_fp64 = factory.build(
            backend="torch",
            device=device,
            dtype=self.config.geometric_dtype,
            validate=False
        )
    ...
    def _get_cached_distances(
        ...
        Returns tensor in runtime dtype on correct device.
        """
        ...
        # Cache miss - compute with proper dtype
        factory = self._get_or_create_factory(seq_len, RouteMode.DISTANCE)
    ...
    def _get_cached_routes(
        ...
        """Get routing table from cache (int64 indices)."""
        ...
        # Build on demand
        routes = self._build_fusion_routes(seq_len, k)
        return routes.to(device=device, dtype=torch.int64)
    ...
    def _build_fusion_routes(self, seq_len: int, k: int) -> torch.Tensor:
        ...
        Uses FP64 distances for precision, returns int64 indices.
        """
        if self.config.use_beatrix_routing:
            # Get Cantor measure in FP64 for precise distance computation
            factory = self._get_or_create_factory(seq_len, RouteMode.STAIRCASE_FEATURES)
            cantor_measure_fp64, _ = factory.build(
                backend="torch",
                device="cpu",
                dtype=self.config.geometric_dtype,
                validate=False
            ...
            distance_matrix = torch.abs(
                cantor_measure_fp64.unsqueeze(1) - cantor_measure_fp64.unsqueeze(0)
            )
        else:
            # Get pre-computed distances (already in FP64)
            factory = self._get_or_create_factory(seq_len, RouteMode.DISTANCE)
        ...
        routes = torch.zeros(seq_len, k, dtype=torch.int64)
    ...
    def _compute_fusion_weights(
        ...
        """Compute fusion weights with proper dtype handling."""
        batch, heads, seq, k, head_dim = x_gathered.shape
        device = x_anchor.device

        if self.config.fusion_mode == "weighted":
            # Use pre-cached distance matrix (already in runtime dtype)
            distance_matrix = self._get_cached_distances(seq, device)

            weights = torch.zeros(seq, k, device=device, dtype=self.config.dtype)
            for i in range(seq):
                weights[i] = distance_matrix[i, routes[i]]

            # Use buffered epsilon for stability
            weights = 1.0 / (weights + self.eps_buffer)
            ...
            weights = torch.ones(batch, heads, seq, k, device=device, dtype=self.config.dtype)
    ...
    def forward(
        ...
        """
        Forward pass with proper buffer and dtype management.
        """
        ...
        # Use buffered residual scale
        residual = x * self.residual_scale_buffer

        # Get Beatrix staircase (instant buffer lookup, already correct dtype/device)
        ...
        # Get routes (instant buffer lookup, int64)
        k = self.config.get_window_size(seq_len)
        routes = self._get_cached_routes(seq_len, k, x.device)

        # Optimized gathering with torch.gather
        ...
        # Residual connection with buffered scale
    ...
    def get_fusion_info(self, seq_len: int) -> Dict:
        ...
        return {
            ...
            'runtime_dtype': self.config.dtype,
            'geometric_dtype': self.config.geometric_dtype,
            'residual_scale': self.residual_scale_buffer.item()
        }
    ...
    def extra_repr(self) -> str:
        ...
        return (f'dim={self.dim}, num_heads={self.num_heads}, '
                ...
                f'residual_scale={self.residual_scale_buffer.item():.2f}')
