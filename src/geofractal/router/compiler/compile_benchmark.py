...
2. torch.compile
3. torch.compile with fullgraph
4. VMap WideRouter (true vectorized batching via torch.func.vmap)
5. VMap WideRouter + torch.compile
...
Usage:
    ...
    python compile_benchmark.py --towers 8 --depth 4 --dim 512 --warmup 10 --iters 100
...
from geofractal.router.compiler.compile_router import (
    CompileRouter,
    CompiledWideRouter,
    compile_module,
)
...
def benchmark_model(
    ...
    # -------------------------------------------------------------------------
    print(f"\n--- Eager Baseline ---")
    original = compiler.get_original()
    results['eager'] = benchmark_forward(
        original, input_tensor, "Eager",
    ...
    # -------------------------------------------------------------------------
    # 2. torch.compile via CompileRouter
    # -------------------------------------------------------------------------
    print(f"\n--- torch.compile (via CompileRouter) ---")
    gc.collect()
    if sync_cuda:
        torch.cuda.empty_cache()

    t0 = time.perf_counter()
    compiled_model = compiler.compile(mode='reduce-overhead')
    with torch.no_grad():
        _ = compiled_model(input_tensor)
        if sync_cuda:
            torch.cuda.synchronize()
    compile_times['compiled'] = time.perf_counter() - t0
    print(f"  Compile: {compile_times['compiled']:.2f}s")

    results['compiled'] = benchmark_forward(
        compiled_model, input_tensor, "Compiled",
        warmup=warmup, iterations=iterations, sync_cuda=sync_cuda,
    )
    print(f"  {results['compiled'].mean_time_ms:.3f}ms ± {results['compiled'].std_time_ms:.3f}ms")

    # -------------------------------------------------------------------------
    # 3. torch.compile with fullgraph (optional)
    # -------------------------------------------------------------------------
    if include_fullgraph:
        ...
        try:
            original = compiler.get_original()
            t0 = time.perf_counter()
            compiled_fg = torch.compile(original, mode='reduce-overhead', fullgraph=True)
            with torch.no_grad():
                _ = compiled_fg(input_tensor)
                if sync_cuda:
                    torch.cuda.synchronize()
            compile_times['fullgraph'] = time.perf_counter() - t0
            print(f"  Compile: {compile_times['fullgraph']:.2f}s")

            results['compiled_fullgraph'] = benchmark_forward(
                compiled_fg, input_tensor, "Compiled fullgraph",
                warmup=warmup, iterations=iterations, sync_cuda=sync_cuda,
            )
            print(f"  {results['compiled_fullgraph'].mean_time_ms:.3f}ms ± {results['compiled_fullgraph'].std_time_ms:.3f}ms")
        except Exception as e:
            print(f"  Failed: {e}")
            results['compiled_fullgraph'] = None

    # -------------------------------------------------------------------------
    # 4. VMap WideRouter (true vectorized batching)
    # -------------------------------------------------------------------------
    print(f"\n--- VMap WideRouter ---")
    gc.collect()
    if sync_cuda:
    ...
    try:
        t0 = time.perf_counter()
        wide_router = compiler.build_wide_router()
        wide_router = wide_router.to(device)
        wide_router.eval()
        build_time = (time.perf_counter() - t0) * 1000

        vmap_groups = compiler.build_vmap_groups()
        num_vmap_groups = len(vmap_groups)

        print(f"  Build: {build_time:.2f}ms")
        print(f"  VMap groups: {num_vmap_groups}")

        if num_vmap_groups > 0:
            # Benchmark the wide router forward
            results['wide_router'] = benchmark_forward(
                wide_router, input_tensor, "WideRouter",
                warmup=warmup, iterations=iterations, sync_cuda=sync_cuda,
            )
            print(f"  {results['wide_router'].mean_time_ms:.3f}ms ± {results['wide_router'].std_time_ms:.3f}ms")

            # Also benchmark compiled wide router
            print(f"\n--- VMap WideRouter + torch.compile ---")
            gc.collect()
            if sync_cuda:
                torch.cuda.empty_cache()

            t0 = time.perf_counter()
            compiled_wide = torch.compile(wide_router, mode='reduce-overhead')
            with torch.no_grad():
                _ = compiled_wide(input_tensor)
                if sync_cuda:
                    torch.cuda.synchronize()
            compile_times['wide_compiled'] = time.perf_counter() - t0
            print(f"  Compile: {compile_times['wide_compiled']:.2f}s")

            results['wide_compiled'] = benchmark_forward(
                compiled_wide, input_tensor, "WideRouter compiled",
                warmup=warmup, iterations=iterations, sync_cuda=sync_cuda,
            )
            print(f"  {results['wide_compiled'].mean_time_ms:.3f}ms ± {results['wide_compiled'].std_time_ms:.3f}ms")
        else:
            print(f"  No batchable groups found - skipping vmap benchmark")
            results['wide_router'] = None
            results['wide_compiled'] = None
    ...
    compiled_ms = results['compiled'].mean_time_ms

    comparison['eager_ms'] = baseline_ms
    comparison['compiled_ms'] = compiled_ms
    comparison['speedup'] = baseline_ms / compiled_ms
    comparison['analysis_overhead_ms'] = introspect_time + staging_time
    comparison['compile_time_s'] = compile_times['compiled']
    ...
    print(f"\n{'Method':<40} {'Time (ms)':<12} {'Speedup':<10}")
    print("-" * 62)
    print(f"{'Eager':<40} {baseline_ms:<12.3f} {'1.00x':<10}")
    print(f"{'torch.compile':<40} {compiled_ms:<12.3f} {comparison['speedup']:<10.2f}x")

    if include_fullgraph and results.get('compiled_fullgraph'):
        fg_ms = results['compiled_fullgraph'].mean_time_ms
        print(f"{'torch.compile (fullgraph)':<40} {fg_ms:<12.3f} {baseline_ms/fg_ms:<10.2f}x")

    if results.get('wide_router'):
        wide_ms = results['wide_router'].mean_time_ms
        print(f"{'VMap WideRouter':<40} {wide_ms:<12.3f} {baseline_ms/wide_ms:<10.2f}x")
        comparison['wide_router_ms'] = wide_ms
        comparison['wide_router_speedup'] = baseline_ms / wide_ms

    if results.get('wide_compiled'):
        wide_comp_ms = results['wide_compiled'].mean_time_ms
        print(f"{'VMap WideRouter + compile':<40} {wide_comp_ms:<12.3f} {baseline_ms/wide_comp_ms:<10.2f}x")
        comparison['wide_compiled_ms'] = wide_comp_ms
        comparison['wide_compiled_speedup'] = baseline_ms / wide_comp_ms

    print(f"\n--- CompileRouter Analysis (informational) ---")
    print(f"Introspection: {introspect_time:.2f}ms")
    print(f"Staging: {staging_time:.2f}ms")
    print(f"Batchable stages found: {stats['batchable_stages']}/{stats['total_stages']}")

    print(f"\n--- Compile Times ---")
    print(f"torch.compile: {comparison['compile_time_s']:.2f}s")
    if 'wide_compiled' in compile_times:
        print(f"VMap WideRouter compile: {compile_times['wide_compiled']:.2f}s")
...
def run_benchmark(args):
    ...
    print("Eager vs torch.compile vs VMap WideRouter")
